...
namespace mylib{
  ...
  Int_t getPartonTag(MCParticle *mcPart) {
    // check whether the parton is after parton shower
    Int_t iFinalParton = 0;
    ...
    if (abs(pdg)<10 || pdg==21)  {
      if (daughterpdg == 92) iFinalParton = 1;
    }
    return iFinalParton;
  }
  ...
  Int_t getMCSerial(ReconstructedParticle *recPart, LCCollection *colMCTL, LCCollection *colMCP) {
    // get the serial code of the particle in the MC particle list
    MCParticle *mcPart = getMCParticle(recPart,colMCTL);
    if (!mcPart) {
      return -1;
    }
    else {
      return getMCSerial(mcPart,colMCP);
    }
  }
  ...
  Int_t getTruthPDG(ReconstructedParticle *recPart, LCCollection *colMCTL) {
    // get the PDG of the linked MCParticle
    ...
    Double_t weight;
    Int_t nlinks;
    Int_t ilink = getLinkedMCParticle(recPart,colMCTL,weight,nlinks);
    if (ilink >= 0) {
      MCParticle *mcPart = dynamic_cast<MCParticle *>(vecMCTL[ilink]);
      truthPDG = mcPart->getPDG();
    }
    delete navMCTL;
    return truthPDG;
  }
  ...
  Int_t getOriginalPDG(Int_t iMin, MCParticle *mcPart, LCCollection *colMC) {
    ...
    while (nParents > 0 && serial > iMin) {
      if (mcPart->isOverlay()) break;
      mcPart = mcPart->getParents()[0];
  ...
  Int_t getOriginalPDGForIsoLep(ReconstructedParticle *recPart, LCCollection *colMCTL, LCCollection *colMC) {
    ...
    if (ilink >= 0) {
      MCParticle *mcPart = dynamic_cast<MCParticle *>(vecMCTL[ilink]);
      origPDG = getOriginalPDGForIsoLep(mcPart,colMC);
    }
    delete navMCTL;
    return origPDG;
  }
  ...
  Int_t getOriginalPDGForIsoLep(ReconstructedParticle *recPart, LCCollection *colMCTL) {
    ...
    if (ilink >= 0) {
      MCParticle *mcPart = dynamic_cast<MCParticle *>(vecMCTL[ilink]);
      truthPDG = getOriginalPDGForIsoLep(mcPart);
    }
    delete navMCTL;
    return truthPDG;
  }
  ...
  Int_t getOriginalSerialForIsoLep(ReconstructedParticle *recPart, LCCollection *colMCTL, LCCollection *colMCP) {
    ...
    if (vecMCTL.size() > 0) {
      MCParticle *mcPart = dynamic_cast<MCParticle *>(vecMCTL[0]);
      originalSerial = getOriginalSerialForIsoLep(mcPart,colMCP);
    }
    delete navMCTL;
    return originalSerial;
  }
  ...
  Int_t getOriginalSerial(Int_t iMin, MCParticle *mcPart, LCCollection *colMCP) {
    // get the serial number of the original particle where the MCParticle comes from
    Int_t nParents = mcPart->getParents().size();
    Int_t serial = getMCSerial(mcPart,colMCP);
    if (mcPart->isOverlay()) return -22;
    while (nParents > 0 && serial > iMin) {
      MCParticle *mother = mcPart->getParents()[0];
      nParents = mother->getParents().size();
      mcPart = mother;
      serial = getMCSerial(mcPart,colMCP);
    }
    return serial;
  }
  ...
  Int_t getOriginalSerialForIsoLep(MCParticle *mcPart, LCCollection *colMCP) {
	...
	if (mcPart->getParents().size() > 0) {
	  MCParticle *mmother = mcPart->getParents()[0];	
	  Int_t mpdg = mmother->getPDG();
	  if (abs(mpdg) == 11 || abs(mpdg)==24) break;
	}
      }
    }
    Int_t originalSerial = getMCSerial(mcPart,colMCP);
  ...
  void doClusteringMC(LCCollection *colMC, LCCollection *colMCTL, LCCollection *colPFO,
    ...
    // color-singlet jet clustering using MC truth information
    // input:
    // serial1: index in mcparticleskimmed list for 1st colorsinglet
    // serial2: index in mcparticleskimmed list for 2nd colorsinglet
    // output:
    // mass1: mass of 1st colorsinglet
    // mass2: mass of 2nd colorsinglet

    MCParticle *mc1 = dynamic_cast<MCParticle *>(colMC->getElementAt(serial1));
    MCParticle *mc2 = dynamic_cast<MCParticle *>(colMC->getElementAt(serial2));

    TLorentzVector lortz1,lortz2;
    Int_t np = colPFO->getNumberOfElements();
    for (Int_t i=0;i<np;i++) {
      ReconstructedParticle *pfo = dynamic_cast<ReconstructedParticle *>(colPFO->getElementAt(i));
      TLorentzVector lortz = TLorentzVector(pfo->getMomentum(),pfo->getEnergy());
      MCParticle *mcp = getMCParticle(pfo,colMCTL);
      if (!mcp) continue;
      if (isFromMCParticle(mcp,mc1)) lortz1 += lortz;
      else if (isFromMCParticle(mcp,mc2)) lortz2 += lortz;
    }
    mass1 = lortz1.M();
    mass2 = lortz2.M();
  }
  ...
  Bool_t isFromMCParticle(MCParticle *mcp,MCParticle *mc) {
    // tag whether mcp is from mc decay

    if (mcp == mc) return kTRUE;
    while (mcp->getParents().size() > 0) {
  ...
  Bool_t isFromMCParticle(MCParticle *mcp,MCParticle *mc1,MCParticle *mc2) {
    // tag whether mcp is from mc1 or mc2 decay

    if (mcp == mc1 || mcp == mc2) return kTRUE;
    while (mcp->getParents().size() > 0) {
      mcp = mcp->getParents()[0];
      if (mcp == mc1 || mcp == mc2) return kTRUE;
    }
  ...
  LCCollection* getPFOsColWithoutISRandOverlay(LCCollection *colMC, LCCollection *colMCTL,
					       ...
					       MCParticle *isr2) {
    // remove ISR and overlay particles from PFOs collection
    // for purpose of developing algorithms foucsing on signal particles

    LCCollectionVec *colNew = new LCCollectionVec(LCIO::RECONSTRUCTEDPARTICLE);
    colNew->setSubset(true);

    Int_t nPFOs = colPFO->getNumberOfElements();
    for (Int_t i=0;i<nPFOs;i++) {
      ReconstructedParticle *pfo = dynamic_cast<ReconstructedParticle*>(colPFO->getElementAt(i));
      MCParticle *mcp = getMCParticle(pfo,colMCTL);
      if (mcp->isOverlay()) continue;
      if(isFromMCParticle(mcp,isr1) || isFromMCParticle(mcp,isr2)) continue;
      colNew->addElement(pfo);
    }
    return colNew;
  }
  ...
  Int_t getColorSingletID(LCCollection *colMC, LCCollection *colMCTL, ReconstructedParticle *jet,MCParticle *mc1,MCParticle *mc2,Double_t &emis) {
    // tag which color singlet the jet is from based on the majority origin of jet energy 
    // return [0]: from first Higgs; [1]: from second Higgs; [2]: from other color-singlet (mostly Z); [3]: from overlay


    Double_t  energyComponents[4] = {0.,0.,0.,0.};
    ...
    for (Int_t i=0;i<np;i++) {
      ...
      if (!mcp) continue;
      Double_t energy = pfo->getEnergy();
      if (isFromMCParticle(mcp,mc1)) { // 1st H
	energyComponents[0] += energy;
      }
      else if (isFromMCParticle(mcp,mc2)) { // 2nd H
	energyComponents[1] += energy;
      }
      else if (mcp->isOverlay()) { // overlay
  ...
  TVector2 getMomentumNew(Double_t px,Double_t py,Double_t theta1,
    ...
    // new method to get momentum magnitude of two particles
    // provided the total directions and total px and py known

    Double_t phi = phi1 - phi2;
    Double_t cosPhi1 = TMath::Cos(phi1);
    Double_t cosPhi2 = TMath::Cos(phi2);
    Double_t cosPhi  = TMath::Cos(phi);
    Double_t sinPhi1 = TMath::Sin(phi1);
    Double_t sinPhi2 = TMath::Sin(phi2);
    Double_t sinPhi  = TMath::Sin(phi);
    //    Double_t cosTheta1  = TMath::Cos(theta1);
    //    Double_t cosTheta2  = TMath::Cos(theta2);
    Double_t sinTheta1  = TMath::Sin(theta1);
    Double_t sinTheta2  = TMath::Sin(theta2);

    Double_t p1 = (cosPhi1-cosPhi*cosPhi2)*px+(sinPhi1-cosPhi*sinPhi2)*py;
    p1 *= 1./sinTheta1/sinPhi/sinPhi;
    Double_t p2 = (cosPhi2-cosPhi*cosPhi1)*px+(sinPhi2-cosPhi*sinPhi1)*py;
    p2 *= 1./sinTheta2/sinPhi/sinPhi;

    return TVector2(p1,p2);
  }
  
  // *******************************************************
  // *******************************************************
  TLorentzVector getChargedJetMomentum(ReconstructedParticle *jet) {
    TLorentzVector lortzJ_C(0,0,0,0);
    Int_t np = jet->getParticles().size();
    for (Int_t j=0;j<np;j++) {
      ReconstructedParticle *pfo = dynamic_cast<ReconstructedParticle *>(jet->getParticles().at(j));
      if (TMath::Abs(pfo->getCharge()) > 0.5) lortzJ_C += getLorentzVector(pfo);
    }
    return lortzJ_C;
  }
