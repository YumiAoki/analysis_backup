...
void IsolatedLeptonTaggingProcessor::processEvent( LCEvent * evt ) { 
 ...
 TLorentzVector lortz2f = lortzf1MC + lortzf2MC;
  ...
  if (!hStatlep) hStatlep = new TH1D("hStatlep", "Cut Table", 20, 0, 20);
  Double_t selid = -0.5;

  if (isoLeptons.size()==0)  throw marlin::SkipEventException(this);
  hStatlep->Fill(++selid);
  //  gCutName[(Int_t)selid] << "E_Photon > 100 && M_Recoil > 600" << ends;
  gCutName[(Int_t)selid] << " n of isolated lepton > 0" << ends;  
